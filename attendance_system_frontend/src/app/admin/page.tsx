...
//         // ✅ non-admin gets redirected to user dashboard
...
//     loadAttendanceLogs(user.employee_id);
//   };
...
//         if (!existing || new Date(log.login_time) > new Date(existing.login_time)) {
//           latestStatus.set(log.user_id, log);
//         }
//       }
//     });
//     return Array.from(latestStatus.values()).filter((l) => l.status === 'logged_in').length;
//   };
...
//   const selectedUserInfo = allUsers.find((u) => u.employee_id === selectedUserId);
...
//           {[
//             { icon: ShieldCheck, label: 'Admin', value: currentUser.name.split(' ')[0], color: 'violet' },
//             { icon: Users, label: 'Total Students', value: String(allUsers.length), color: 'cyan' },
//             { icon: CheckCircle, label: 'Present Today', value: String(getLoggedInCount()), color: 'emerald' },
//             { icon: Clock, label: "My Hours Today", value: getTotalWorkingHours(), color: 'blue' },
...
//                       selectedUserId === user.employee_id
...
//                     {selectedUserInfo ? `${selectedUserInfo.name}'s Attendance` : 'Select a student'}
//                   </h2>
//                   {selectedUserInfo && (
...
//       }
//     });
//     const present = Array.from(latestByUser.values()).filter(l => l.status === 'logged_in' || l.status === 'present').length;
//     const loggedOut = Array.from(latestByUser.values()).filter(l => l.status === 'logged_out').length;
//     const absent = allUsers.length - present - loggedOut;
//     const late = Array.from(latestByUser.values()).filter(l => new Date(l.login_time).getHours() >= LATE_THRESHOLD_HOUR).length;
//     return { present, loggedOut, absent: Math.max(0, absent), late, total: allUsers.length };
//   }, [allLogs, allUsers, today]);
...
//       allLogs
//         .filter(l => new Date(l.login_time).toDateString() === today && (l.status === 'logged_in' || l.status === 'present'))
//         .map(l => l.user_id)
//     );
//     allUsers.forEach(u => {
...
//       }
//     });
//     return Array.from(map.entries()).map(([name, v]) => ({ name, ...v, absent: v.total - v.present }));
//   }, [allLogs, allUsers, today]);
...
//         absent: Math.max(0, allUsers.length - presentSet.size),
//       });
//     }
//     return days;
//   }, [allLogs, allUsers]);
...
//         const ds = d.toDateString();
//         const present = allLogs.some(l => l.user_id === u.id && new Date(l.login_time).toDateString() === ds);
...
//     });
//     return list;
//   }, [studentAttendance, allUsers, allLogs]);

//   // ── Filtered students ─────────────────────────────────
//   const departments = useMemo(() => ['all', ...Array.from(new Set(allUsers.map(u => u.department).filter(Boolean)))], [allUsers]);

//   const presentTodayIds = useMemo(() => new Set(
//     allLogs.filter(l => new Date(l.login_time).toDateString() === today && (l.status === 'logged_in' || l.status === 'present')).map(l => l.user_id)
//   ), [allLogs, today]);

//   const filteredUsers = useMemo(() => allUsers.filter(u => {
//     const q = searchQuery.toLowerCase();
//     const matchQ = !q || u.name.toLowerCase().includes(q) || u.employee_id.toLowerCase().includes(q) || u.department?.toLowerCase().includes(q);
...
//   }), [allUsers, searchQuery, deptFilter, statusFilter, presentTodayIds]);

//   const selectedUserInfo = allUsers.find(u => u.employee_id === selectedEmployeeId);

//   // ── My working hours ──────────────────────────────────
//   const myHoursToday = useMemo(() => {
//     const logs = myLogs.filter(l => new Date(l.login_time).toDateString() === today);
//     let mins = 0;
//     logs.forEach(l => { if (l.logout_time) mins += calcDuration(l.login_time, l.logout_time)?.total ?? 0; });
//     return `${Math.floor(mins / 60)}h ${Math.floor(mins % 60)}m`;
//   }, [myLogs, today]);

//   // ── CSV Export ────────────────────────────────────────
//   const handleExport = () => {
//     const logsToExport = selectedLogs.length > 0 ? selectedLogs : allLogs;
//     const header = 'Employee ID,Name,Date,Login,Logout,Duration,Status\n';
...
//             { key: 'alerts', icon: AlertTriangle, label: `Alerts ${alerts.length > 0 ? `(${alerts.length})` : ''}` },
...
//               <div>
//                 <h1 className="text-2xl font-extrabold tracking-tight">Good {getGreeting()}, {currentUser.name.split(' ')[0]} 👋</h1>
...
//               <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
//                 {[
//                   { icon: Users, label: 'Total Students', value: todayStats.total, color: 'cyan', sub: 'registered' },
//                   { icon: UserCheck, label: 'Present Today', value: todayStats.present, color: 'emerald', sub: `${Math.round((todayStats.present / todayStats.total) * 100) || 0}% rate` },
//                   { icon: UserX, label: 'Absent Today', value: todayStats.absent, color: 'red', sub: 'not checked in' },
//                   { icon: Clock, label: 'Late Arrivals', value: todayStats.late, color: 'amber', sub: `after ${LATE_THRESHOLD_HOUR}:00 AM` },
//                 ].map(({ icon: Icon, label, value, color, sub }) => (
//                   <div key={label} className="rounded-2xl border border-white/10 bg-white/5 p-5 flex flex-col gap-3 hover:bg-white/8 transition-all">
//                     <div className={`w-9 h-9 rounded-lg flex items-center justify-center ${
//                       color === 'cyan' ? 'bg-cyan-500/15 text-cyan-400'
...
//                     <PieChart>
//                       <Pie data={deptData} dataKey="present" nameKey="name" cx="50%" cy="50%" outerRadius={70} label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`} labelLine={false}>
...
//                     {allUsers.filter(u => !presentTodayIds.has(u.id)).slice(0, 20).map(u => (
//                       <div key={u.id} className="flex items-center justify-between py-1.5 px-3 rounded-lg bg-white/5 hover:bg-white/8 transition-all">
//                         <div>
//                           <p className="text-sm font-medium">{u.name}</p>
//                           <p className="text-xs text-slate-500">{u.employee_id} · {u.department || 'No dept'}</p>
//                         </div>
//                         <span className="text-xs text-red-400 bg-red-500/10 px-2 py-0.5 rounded-full">Absent</span>
//                       </div>
//                     ))}
//                     {todayStats.absent === 0 && (
...
//                       <Users className="w-4 h-4 text-cyan-400" /> Students
//                     </h2>
...
//                     >
//                       {departments.map(d => <option key={d} value={d} className="bg-[#0d1424]">{d === 'all' ? 'All Depts' : d}</option>)}
//                     </select>
//                     <select
//                       value={statusFilter}
...
//                         className={`w-full px-4 py-3 flex items-center justify-between hover:bg-white/5 transition-all text-left ${
//                           selectedEmployeeId === user.employee_id ? 'bg-cyan-500/10 border-l-2 border-cyan-500' : ''
//                         }`}
//                       >
...
//                             )}
//                           </div>
//                           <p className="text-xs text-slate-500 truncate">{user.employee_id} · {user.department || 'No dept'}</p>
//                         </div>
...
//                             return (
//                               <div>
//                                 <p className={`text-3xl font-extrabold ${pct < LOW_ATTENDANCE_THRESHOLD ? 'text-red-400' : 'text-emerald-400'}`}>
//                                   {pct}%
//                                 </p>
...
//                     Attendance Leaderboard (30 days)
//                   </h2>
//                 </div>
//                 <div className="divide-y divide-white/5 max-h-96 overflow-y-auto">
//                   {[...studentAttendance].reverse().map((u, i) => (
//                     <div key={u.id} className="px-5 py-3 flex items-center gap-4 hover:bg-white/5 transition-all">
...
//                           <div
//                             className={`h-1.5 rounded-full ${u.attendancePct >= 75 ? 'bg-cyan-400' : 'bg-red-400'}`}
...
//                         <p className="text-xs text-slate-400 mt-0.5">{alert.user.employee_id} · {alert.user.department}</p>
//                         <p className={`text-xs font-medium mt-1 ${alert.type === 'low' ? 'text-amber-400' : 'text-red-400'}`}>
//                           ⚠ {alert.detail}
//                         </p>
//                       </div>
//                       <button
...
//                 <td className="px-5 py-3 text-sm text-slate-400 font-mono">{log.logout_time ? formatTime(log.logout_time) : '—'}</td>
//                 <td className="px-5 py-3 text-sm text-slate-300">{dur ? `${dur.hrs}h ${dur.mins}m` : '—'}</td>
